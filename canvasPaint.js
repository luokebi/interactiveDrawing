...
(function($) {
    ...
    $.fn.canvasPaint = function(method) {
        ...
        var state = {
            ...
            moved: false,
            shadow: true,
            lasttools: TOOLS.EMPTY_RECTANGLE,
            tool: TOOLS.EMPTY_RECTANGLE,
            lineThickness: 10,
            strokeColor: "#FF0000",
            fillColor: "#FF0000",
            sprayDensity: 5,
...
//                canvas.cache.data = canvas.fon.context.getImageData(0, 0, canvas.common.width, canvas.common.height);
        ...
        function setColorProperty(c) {
            var styler = $('#colortools-styler');
            styler.find('.text').css('background-color', c);
            styler.find('li.selected.sel').removeClass('selected sel');
            var myclass = $('#colortools').find('option[value=' + c + ']').attr('class');
            styler.find('li.' + myclass).addClass('selected sel');
        }
        ...
        var ObTools = {
            ...
            setDrawOptions: function() {
                ...
                if (localStorage.enableshadow !== 'false') {
            ...
            drawNumber: function() {
                if (localStorage.enablenumbers !== 'false') {
            ...
            checkNumber: function(x, y) {
                if (localStorage.enablenumbers === 'false') return false;
        ...
        function ObRectangle() {
            ...
            this.draw = function(context) {
                ...
                this.setDrawOptions();
                if (this.width == 1){
                    this.context.strokeRect(this.x + 0.5, this.y+0.5, this.w, this.h);
                } else {
...
//                this.drawNumber();
            ...
            this.createSticker = function() {
                ...
                textarea.css({'width': w + 'px', 'height': h + 'px', 'top': y, 'left': x});
                textarea.focus();
            ...
            this.drawTextSticker = function() {
                ...
                wrapText(ctx, this.text, 0, 0, w, w / 2 + 2, -1, 14);
...
//                }
                ...
                wrapText(ctx, this.text, 0, 0, this.w, 0, 0, this.size);

                this.context = saveCtx;
                this.context.save();
                this.context.shadowColor = 'transparent';
                this.context.drawImage(canvas, this.x, this.y - 2);
                this.context.restore();
            };
            this.setDefaultValue = function() {
...
//                                _text.h +=_text.size;
//                                _text.hLast = _text.h;
//                                textarea.css({'height': _text.h + 'px'});
//                                _text.drawMarks();
                            }
                        }
                    });
                ...
                textarea.css({'width': this.w + 'px', 'height': this.h + 'px', 'top': this.y, 'left': this.x, 'color': this.color, 'font-size': this.size + 'px'});
                textarea.focus();
...
//                this.drawNumber();
            ...
            this.move = function(dx, dy) {
                ...
                if(this.marker == 2 || this.marker == 4) {
                    this.xb1 = this.xb1Last + dx;
                    if ((this.xb2 - this.xb1) < 63) {
                        this.xb1 = this.xb2 - 63;
                    }
                }

                if(this.marker == 2 || this.marker == 3) {
                    this.yb1 = this.yb1Last + dy;
                    if ((this.yb2 - this.yb1) < 63){
                        this.yb1 = this.yb2 - 63;
                    }
                }
...
//                    }
        ...
        var drawStart = function(event) {
            ...
            if (localStorage.enableshadow !== 'false') {
                canvas.current.context.shadowColor = '#333';
                canvas.current.context.shadowBlur = 5;
            ...
            switch (state.tool) {
                ...
                case TOOLS.PEN:
                    ...
                    points = [state.startDrawX, state.startDrawY];
                    canvas.current.context.shadowColor = 'transparent';
                ...
                case TOOLS.TEXT:
                    ...
                    if (textarea) {
                        ...
                        currentObject.setValue(state.startDrawX - 4, state.startDrawY - (state.lineThickness * 5) / 2, state.lineThickness, state.strokeColor);
                        currentObject.size = state.lineThickness * 5;
                        currentObject.h = state.lineThickness * 5 + 4;
            ...
            if ('setValue' in currentObject && state.tool !== TOOLS.EDIT) {
                if (state.tool !== TOOLS.STICKER && state.tool !== TOOLS.TEXTARROW) {
                    currentObject.number.n = getNumber();
                }
            ...
            if (state.drawing) {
                ...
                switch (state.tool) {
                    ...
                    case TOOLS.ELLIPSE:
                        ...
                        currentObject.w = x - state.startDrawX;
                        currentObject.h = y - state.startDrawY;
                        currentObject.wLast = x - state.startDrawX;
                        currentObject.hLast = y - state.startDrawY;
                        currentObject.draw(canvas.current.context);
                    ...
                    case TOOLS.PEN:
                        ...
                        canvas.current.context.moveTo(points[0], points[1]);
...
//                        currentObject.draw(canvas.current.context);
                    ...
                    case TOOLS.TEXTARROW:
                        ...
                        currentObject.xb1Last = currentObject.xb1 = state.startDrawX - 92;
                        currentObject.yb1Last = currentObject.yb1 = state.startDrawY - 46;
                        currentObject.xb2Last = currentObject.xb2 = state.startDrawX;
                        currentObject.yb2Last = currentObject.yb2 = state.startDrawY + 46;
...
//            }
        ...
        this.setToolsWigth = function(w) {
            var c = JSON.parse(localStorage.setting).color;
            localStorage.setting = JSON.stringify({width: w, color: c});
        };

        this.setToolsColor = function(c) {
            var w = JSON.parse(localStorage.setting).width;
            localStorage.setting = JSON.stringify({width: w, color: c});
        };
